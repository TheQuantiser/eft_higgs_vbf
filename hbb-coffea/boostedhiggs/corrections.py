...
#             pair = ak.concatenate([var_weights[:, 0:1], var_weights[:, 7:8]], axis=1)
#             up = ak.max(pair, axis=1)
#             down = ak.min(pair, axis=1)
#         elif len(var_weights[0]) > 1:
#             print("Scale variation vector has length ", len(var_weights[0]))
...
def add_scalevar_3pt(weights, var_weights):
    print('trying add_scalevar_3pt')
    n = len(weights.weight())
    nom = np.ones(n, dtype=np.float32)
    up = np.ones(n, dtype=np.float32)
    down = np.ones(n, dtype=np.float32)

    #adds 1's if none
    if var_weights is None or (isinstance(var_weights, (list, tuple)) and len(var_weights) == 0):
        weights.add("scalevar_3pt", nom, up, down)
        return

    #per-event vector length (MG has 8, Powheg has 9)
    veclen = ak.to_list(ak.max(ak.num(var_weights, axis=1)))
    if veclen not in (8, 9):
        print("Scale variation vector does not have length 8 or 9, but:", veclen)
        weights.add("scalevar_3pt", nom, up, down)
        return

    #creates pairs of var_weight points corresponding to (0.5,0.5) and (2,2) the extreme choices of µ_r and µ_F
    if veclen == 9:
        pair = ak.concatenate([var_weights[:, 0:1], var_weights[:, 8:9]], axis=1)
    else: #for MG veclen == 8
        pair = ak.concatenate([var_weights[:, 0:1], var_weights[:, 7:8]], axis=1)
 
    up = ak.to_numpy(ak.max(pair, axis=1)).astype(np.float32, copy=False)
    down = ak.to_numpy(ak.min(pair, axis=1)).astype(np.float32, copy=False)
