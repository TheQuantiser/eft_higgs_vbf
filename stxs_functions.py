...
def stxs_fit(coffea_name, operator_list, verbose = False):
    ...
    if len(operator_list) == 2:
        ...
        for i1 in np.linspace(-100,100,500):
            for i2 in np.linspace(-100,100,500):
                wcpoint = (i1,i2)
...
def one_sigma_1d(wc_space_dict, delta_chi2=1.0):
    wc_space_dict = {
        (p if isinstance(p, tuple) else (p,)): v
        for p, v in wc_space_dict.items()
    }
    ...
    current_region = []
    for p in points:
        if wc_space_dict[p]['total'][1] <= chi2_min + delta_chi2:
            current_region.append(list(p))
        else:
            if current_region:
                allowed_regions.append(current_region)
                current_region = []
    if current_region:
        allowed_regions.append(current_region)

    allowed_points = [p for region in allowed_regions for p in region]

    return best_point, allowed_points, allowed_regions

def two_sigma_1d(wc_space_dict, delta_chi2=2.0):
    wc_space_dict = {
        (p if isinstance(p, tuple) else (p,)): v
        for p, v in wc_space_dict.items()
    }
...
def operator_sigma_1d(coffea_name, operator_list):
    results = {}

    for op in operator_list:
        wc_space_dict = stxs_fit(coffea_name, [op])
        best, allowed_points, allowed_regions = one_sigma_1d(wc_space_dict)
